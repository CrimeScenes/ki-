...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        local BodyPart
        if shared.Forbidden.PointAssist.ClosestPart then
            BodyPart = GetClosestBodyPart(TargetPlayer.Character)
        else
            BodyPart = TargetPlayer.Character:FindFirstChild(shared.Forbidden.PointAssist.HitPart)
        end
        ...
        if BodyPart then
            ...
            if shared.Forbidden.PointAssist.Aligner then
                ...
                if humanoid then
                    ...
                    predictedPosition = BodyPart.Position + (moveDirection * shared.Forbidden.VelocityCompensation)
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * shared.Forbidden.VelocityCompensation)
            end
...
local function updateFov(TargetPlayer)
    local settings = getgenv().Silent.BoudingSizes  
    clearFovParts()  
...
local function isPartInFovAndVisible(part)
    if not getgenv().PointAssist.Enabled or not IsTargeting or not TargetPlayer then
        return false
    end
...
UserInputService.InputEnded:Connect(function(input, isProcessed)
    ...
    if input.KeyCode == Enum.KeyCode[getgenv().Target.Keybind:upper()] and PointAssist.mode == "hold" then
...
RunService.RenderStepped:Connect(function()
    ...
    if getgenv().Silent.Enabled and IsTargeting then  
        UpdateFOV()  
